...
const PatientListScreen = ({ navigation }) => {
	...
	const handleSearch = (text) => {
		...
		const filtered = patients.filter((patient) => {
			...
			return searchTerms.every((term) => patientName.includes(term) || patientMRN.includes(term));
		});
	...
	return (
		<View style={styles.container}>
			...
			<ScrollView
				...
				refreshControl={
					<RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={[Colors.lightNavalBlue]} tintColor={Colors.lightNavalBlue} />
				}
			>
				{loading ? (
					...
					<View style={styles.emptyStateContainer}>
						...
						<Text style={styles.emptyStateSubtext}>Tap the button below to add your first patient</Text>
					</View>
					...
					filteredPatients.map((patient) => (
						<TouchableOpacity key={patient.mrn} onPress={() => navigation.navigate("PatientDetail", { patient })} style={styles.patientCardWrapper}>
							<PatientCard patient={patient} formatDate={formatDate} minimal={true} />
						</TouchableOpacity>
					))
				)}
			</ScrollView>
