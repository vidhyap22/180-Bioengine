...
const TestScreen = ({ navigation, route }) => {
	...
	const handleBackPress = () => {
		if (recording || stereoRecording) {
			openDialog({
				title: "Leave Test?",
				message: "Any unsaved recordings will be lost.",
				actions: [
					{
						label: "Cancel",
						variant: "cancel",
	...
	const moveToRecordingStep = () => {
		...
		if (selectedDevice.capabilities && !selectedDevice.capabilities.stereo) {
			openDialog({
				title: "Warning",
				message: "The selected device may not support stereo recording, which is required for proper nasalance measurement.",
				actions: [
					{
						label: "Cancel",
						variant: "cancel",
	...
	const startRecording = async () => {
		...
		try {
			...
			Toast.error(error?.message ? `Failed to start recording: ${error.message}` : "Failed to start recording.");
		}
	};
	...
	const saveTestResults = async () => {
		try {
			if (!nasalRecording || !oralRecording || nasalanceScore === null) {
				Toast.error("Processing must complete before saving.");
				return;
			}
					...
					<View key={index} style={styles.stepContainer}>
						<View style={[styles.stepDot, currentStep === index ? styles.activeDot : null, currentStep > index ? styles.completedDot : null]}>
							{currentStep > index ? (
								...
								<Text style={currentStep === index ? styles.activeStepNumber : styles.stepNumber}>{index + 1}</Text>
							)}
						</View>
						<Text style={[styles.stepLabel, currentStep === index ? styles.activeStepLabel : null]}>{step}</Text>
					</View>
				))}
			</View>
		);
	...
	const renderDeviceItem = ({ item }) => {
		...
		return (
			<TouchableOpacity style={[styles.deviceItem, isSelected && styles.deviceItemSelected]} onPress={() => selectAudioDevice(item)}>
				<View style={styles.deviceIconContainer}>
					{item.type === "usb" && <Ionicons name="usb" size={20} color={isSelected ? "#fff" : Colors.lightNavalBlue} />}
				...
				<View style={styles.deviceInfoContainer}>
					<Text style={[styles.deviceName, isSelected && styles.deviceNameSelected]}>{item.name}</Text>

					<Text style={[styles.deviceType, isSelected && styles.deviceTypeSelected]}>
	...
	const renderDeviceSelection = () => {
		return (
			<View style={styles.deviceSelectionContainer}>
				...
				<Text style={styles.instructions}>Select a stereo audio device for recording.</Text>
				...
				{selectedDevice ? (
					<View style={styles.selectedDeviceContainer}>
						...
						<TouchableOpacity style={styles.changeDeviceButton} onPress={() => setDeviceSelectorVisible(true)}>
							<Text style={styles.changeDeviceText}>Change</Text>
						</TouchableOpacity>
					</View>
				) : (
					<TouchableOpacity style={styles.selectDeviceButton} onPress={() => setDeviceSelectorVisible(true)}>
          ...
          <View style={styles.tipContainer}>
            ...
            <Text style={styles.tipText}>Position one microphone near the nose and one near the mouth</Text>
				...
				<TouchableOpacity
					style={[styles.continueButton, !selectedDevice && styles.buttonDisabled]}
		...
		return (
			<View style={styles.recordingContainer}>
				<View style={styles.microphoneInfo}>
					...
					<Text style={styles.instructions}>Position both microphones (nasal and oral) and record the subject reading a provided passage.</Text>
				</View>

				<View style={styles.selectedDeviceReminder}>
					<Text style={styles.selectedDeviceReminderText}>Recording with: {selectedDevice?.name || "Unknown Device"}</Text>
				</View>

				<View style={styles.timerContainer}>
					<Text style={styles.timer}>{formatTime(timer)}</Text>
					{recording && <Animated.View style={[styles.recordingIndicator, { transform: [{ scale: pulseAnim }] }]} />}
				</View>
		...
		return (
			<View style={styles.processingContainer}>
				<View style={styles.processingContent}>
					...
					<Text style={styles.processingSubtext}>Splitting stereo recording into nasal and oral channels...</Text>
				</View>
			</View>
		);
	};
			...
			<View style={styles.reviewContainer}>
				...
				{!isMrnValid && (
					<View style={styles.warningContainer}>
						...
						<Text style={styles.warningText}>Exit "Test Microphones" to save results.</Text>
					</View>
				)}
				...
				<View style={styles.navigationRow}>
					<TouchableOpacity
						style={[styles.saveButton, (loading || !isMrnValid) && styles.disabledButton]}
	...
	return (
		<SafeAreaView style={styles.container}>
			<HeaderBar title="Nasalance Test" onBack={handleBackPress} />
			...
			<Modal visible={deviceSelectorVisible} animationType="slide" transparent={true} onRequestClose={() => setDeviceSelectorVisible(false)}>
				<View style={styles.modalContainer}>
					<View style={styles.modalContent}>
						<View style={styles.modalHeader}>
							<Text style={styles.modalTitle}>Select Audio Device</Text>
							<TouchableOpacity style={styles.modalCloseButton} onPress={() => setDeviceSelectorVisible(false)}>
						...
						{audioDevices.length > 0 ? (
							...
							<View style={styles.noDevicesContainer}>
								...
								<Text style={styles.noDevicesSubtext}>Connect a USB or Bluetooth audio device and try again</Text>

								<TouchableOpacity style={styles.refreshButton} onPress={refreshDeviceList}>
