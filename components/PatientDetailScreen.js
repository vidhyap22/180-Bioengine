...
const PatientDetailScreen = ({ route, navigation }) => {
	...
	const arrayToCSV = (data) => {
		return data.map((row) => row.map((field) => (typeof field === "string" && field.includes(",") ? `"${field}"` : field)).join(",")).join("\n");
	};
	...
	const confirmDelete = () => {
		openDialog({
			...
			message: "Are you sure? This will permanently delete all patient data and cannot be undone.",
			actions: [
				{
					label: "Cancel",
					variant: "cancel",
	...
	return (
		<View style={styles.container}>
			<View style={styles.header}>
				...
				<TouchableOpacity style={styles.moreButton} onPress={showEditOptions}>
					<Ionicons name="ellipsis-vertical" size={24} color={Colors.lightNavalBlue} />
				</TouchableOpacity>
			</View>

			<ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
				<PatientCard patient={patientData} formatDate={formatDate} />
				...
				<View style={styles.statsCard}>
					...
					<View style={styles.statItem}>
						<Text style={styles.statNumber}>{averageNasalance ? `${averageNasalance}%` : "---"}</Text>
				...
				<View style={styles.sectionCard}>
					...
					{loading ? (
						...
						<View style={styles.chartPlaceholder}>
							...
							<Text style={styles.emptyStateSubtext}>Start a new test by tapping the button below</Text>
						</View>
					) : (
						testHistory.map((test) => (
							<TouchableOpacity key={test.id} style={styles.testCard} onPress={() => navigation.navigate("TestDetail", { test })}>
								<View style={styles.testInfo}>
									<Text style={styles.testDate}>{formatDate(test.created_at)}</Text>
									<Text style={styles.testDetails}>
										Duration: {formatDurationAlt(getDuration(test))} • Nasalance: {test.avg_nasalance_score?.toFixed(1) || "N/A"}%
									</Text>
								</View>
				...
				<View style={styles.sectionCard}>
					<View style={styles.sectionHeader}>
						...
						{savingNotes && <Text style={styles.savingIndicator}>Saving...</Text>}
					</View>
					<TextInput
